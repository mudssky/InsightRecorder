...
- 依赖与工具：`drivelist`（设备枚举）、`chokidar`（目录监听）、`fast-glob`（扫描）、`drizzle-orm` + SQLite。
...
- SQLite 初始化与 drizzle-orm 模型与迁移（`AppSettings`/`Device`/`ExportHistory`/`ExportIndex`）
...
- 备份与迁移（数据库备份到用户指定目录；drizzle-orm Migrations 管理结构升级）

## 顺序执行步骤

1. 建模与迁移
   - 定义 drizzle-orm schema（`AppSettings`/`Device`/`ExportHistory`/`ExportIndex`，含唯一约束与索引）。
...
9. 备份与迁移
   ...
   - 使用 drizzle-orm Migrations 管理后续结构升级与数据迁移。
